...
**Beaumont.** Beaumont covers 4.0 km² and had 10,787 inhabitants in 2022. Located at the foothills of the volcanic uplands of the *Plateau des Dômes* and near the Limagne plain, Beaumont is structured by the Artière valley and its alluvial deposits, at the interface with sedimentary formations (marls, clays, and limestones). In this setting, the presence of clay-rich and marl layers (often described as *argilo-calcareous* units) is consistent with sensitivity to clay shrink–swell (RGA), which is triggered by alternating drought and rehydration episodes. The *Fonds Prévention Argile* classifies Beaumont as *high* RGA exposure ([Fonds Prévention Argile: Risques Retrait-Gonflement à Beaumont](https://fonds-prevention-argile.beta.gouv.fr/rga/commune/beaumont-63032)). 

This classification is corroborated by a long and recurrent Cat-Nat recognition history for drought-related ground movements. Beaumont has been recognised under the Cat-Nat drought regime nine times since 1989: for the periods 1989–1991, 1992–1998, 1999–2000 (the corresponding arrêté was published in 2002 but subsequently suspended), 2001, 2016, 2018, 2019, 2020, and 2023 ([Ville de Beaumont: Les mémoires de l’eau - Beaumont](https://www.beaumont63.fr/IMG/pdf/beaumont2009-eau.pdf)). This density of recognitions - spanning more than three decades and covering both the exceptional drought sequences of the early 1990s and the more recent episodes of 2018–2020 - makes Beaumont a natural case study to document within-municipality RGA heterogeneity and its potential implications for pricing and underwriting.
...
**Design logic: a controlled “plan d’expérience”.** Because we observe *quotes* rather than portfolio or claims data, we treat the data collection as a controlled experiment in the sense of general insurance pricing practice ([Parodi: *Pricing in General Insurance*](https://www.taylorfrancis.com/books/mono/10.1201/9781003168881/pricing-general-insurance-pietro-parodi)). We hold fixed (i) the insured profile, (ii) dwelling characteristics, and (iii) the coverage/franchise/option choices within each insurer’s quotation journey; the only factor that varies by construction is *location*. The purpose is to attribute any change in price or offer availability to geographic risk segmentation rather than to observable policyholder heterogeneity. This approach is close in spirit to audit and mystery-shopping designs used to detect differential treatment when observables are controlled by the researcher.
...
The invariance of the insured profile across all quote requests - established by construction, as described in the previous section - ensures that any observed difference in quoted premiums or quote availability between two addresses reflects geographic segmentation rather than observable policyholder or dwelling heterogeneity. The remaining identification challenge is to attribute such geographic differences specifically to RGA or flood exposure, rather than to some other location-varying factor used by insurers ([Moriah et al.: *Contributions of geolocated weather and building-related data for insurance assessment of flood risks*](https://arxiv.org/abs/2603.02418)).

We address this through a series of semi-structured interviews conducted with pricing officers at French household-insurance companies collectively representing more than 50% of the French market by share market volume. All interviewees confirmed that their companies use geographic rating zones for multiple perils - including non-climatic ones - to adjust premiums and underwriting decisions. Crucially, however, all interviewees indicated that the only rating zones operating at the level of the individual building or parcel are those associated with RGA and/or riverine flooding. All other rating zones - covering perils such as theft, fire, or liability - operate at coarser geographic scales, typically the commune or the IRIS.

This finding has a direct implication for our identification strategy. If two addresses belonging to the same commune or the same IRIS receive different premiums or different quoting treatment from the same insurer, the granular geographic variation is, by the interviewees' own account, attributable to RGA or flood rating zones rather than to any other geographic rating layer. Our within-commune and within-IRIS comparisons are therefore interpretable as evidence of RGA- or flood-driven segmentation.

Two geographic features of our case-study communes reinforce this interpretation. Beaumont comprises four IRIS units, and the selected addresses are distributed across them. However, an analysis of INSEE socio-economic indicators reveals no meaningful differences across these four IRIS in Beaumont, so the within-commune premium contrasts we observe are unlikely to reflect socio-economic compositional differences between IRIS. Guipry-Messac, by contrast, constitutes a single commune and a single IRIS, so any within-commune premium contrast there is, by construction, also a within-IRIS contrast, further sharpening the identification.

### Cleaning and Construction of Analysis Objects

- **Within-insurer comparisons only.** Household-insurance contracts are not standardised across insurers: deductibles, assistance services, coverage limits, and optional guarantees differ. Raw premium levels are therefore not compared across insurers. All intensive-margin comparisons are within-insurer ([Mildenhall and Major: *Pricing Insurance Risk: Theory and Practice*](https://onlinelibrary.wiley.com/doi/book/10.1002/9781119756538)).

- **Exposure coding and boundary avoidance.** Each address is assigned to an exposure class using the relevant public hazard map: BRGM exposure classes for RGA, and official flood-zone delineations for the flood contrast. To reduce sensitivity to insurer geocoding differences or parcel-level idiosyncrasies, addresses were chosen to avoid locations near exposure-class boundaries where possible.

- **Cluster averaging.** To mitigate the risk that a single address drives results because of a geocoding error or an unobserved cadastral specificity, outcomes are aggregated at the cluster level. For each insurer and each exposure class, we compute (i) the quote rate (number of issued quotes out of five addresses) and (ii) the mean quoted premium across addresses for which a quote was issued. This cluster-level averaging is the primary robustness device built into the design ([Parodi: *Pricing in General Insurance*](https://www.taylorfrancis.com/books/mono/10.1201/9781003168881/pricing-general-insurance-pietro-parodi)).

- **Treatment of non-quotes.** Non-quotes are recorded as a separate availability indicator for each insurer–cluster pair. Premium statistics are always computed conditional on a quote being issued. Non-quotes enter the analysis through the availability margin and are not imputed as premiums.

### Robustness Checks

Given the small sample sizes inherent to a controlled quote design, robustness is assessed through re-aggregation choices and alternative specifications rather than asymptotic inference.

**Alternative aggregators.** Cluster-level outcomes are recomputed using the median premium instead of the mean, to reduce sensitivity to occasional outlier quotes. The full within-cluster premium range is also reported to document operational dispersion.

**Leave-one-out (jackknife) across addresses.** For each cluster of five addresses, we recompute the cluster premium after removing one address at a time. This produces a simple influence diagnostic: if a pricing-granularity result disappears when a single address is removed, it is more likely to reflect an idiosyncratic geocoding artefact than a structural rating rule.

**Alternative exposure definitions.** We test the stability of results to alternative exposure codings:

- **RGA**: (medium + high) vs. (none + low), as in the main specification; and a stricter split of (high only) vs. (none + low).
- **Flooding**: inside vs. outside the mapped flood zone (main specification); and distance-based bins to the nearest main river segment, where GIS layers permit consistent construction.

**Alternative coding of the availability margin.** We verify that conclusions about withdrawal are not sensitive to the interpretation of non-quotes by (i) treating non-quotes as missing observations (availability analysis only) and (ii) re-expressing availability as a binary outcome in simple descriptive specifications with cluster-level exposure indicators.

**Premium transformation.** Where premiums are available, we replicate the main results using log-premiums and within-insurer normalised premiums (each premium divided by the insurer's mean premium across all issued quotes in the peril-specific sample), to ensure findings are not an artefact of premium scale.
